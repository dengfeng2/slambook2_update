...
void bundleAdjustment(const vector<cv::Point3f> &points_3d,
                      const vector<cv::Point3f> &points_2d, cv::Mat &R,
                      cv::Mat &t);
...
int main(int argc, char *argv[]) {
  ...
  // 建立3D点
  cv::Mat depth1 =
      imread(argv[3], cv::IMREAD_COLOR); // 深度图为16位无符号数，单通道图像
  cv::Mat depth2 =
      imread(argv[4], cv::IMREAD_COLOR); // 深度图为16位无符号数，单通道图像
  cv::Mat K =
...
void bundleAdjustment(const vector<cv::Point3f> &pts1,
                      const vector<cv::Point3f> &pts2, cv::Mat &R, cv::Mat &t) {
  // 构建图优化，先设定g2o
